  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {submitStatus === 'error' && (
          <div className={`px-4 py-3 rounded-lg ${errorMessage?.includes('bereits') ? 'bg-amber-50 border border-amber-200 text-amber-700' : 'bg-red-50 border border-red-200 text-red-600'}`}>
            <div className="flex items-start gap-2">
              ...
              <div>
                <span className="font-medium block">{errorMessage || 'Fehler beim Absenden. Bitte versuchen Sie es erneut.'}</span>
                {errorMessage?.includes('bereits') && (
                  <span className="text-sm mt-1 block">Sie können den Anbieter direkt kontaktieren, um Ihre Bewerbung zu aktualisieren.</span>
                )}
              </div>
            </div>
          </div>
